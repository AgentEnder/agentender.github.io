...
`,s.jsxs(n.p,{children:["After a bit, Nx grew apart from Angular CLI. It became a standalone tool that could be used with any framework. This meant that we needed to create our own configuration file. We created ",s.jsx(n.code,{children:"workspace.json"})," to replace ",s.jsx(n.code,{children:"angular.json"}),". The configuration was extraordiarily similar, but it was a different file and a few properties had a different name. The names changed to more closely match the names that Nx uses. For example, ",s.jsx(n.code,{children:"builder"})," became ",s.jsx(n.code,{children:"executor"})," and ",s.jsx(n.code,{children:"architect"})," became ",s.jsx(n.code,{children:"targets"}),"."]}),`
...
`]}),`
`,s.jsx(n.p,{children:"Additionally, there were other plugin APIs to extend the project graph that were a bit confusing when deciding if a plugin should be an inference plugin or a project graph plugin. Project graph plugins exported a single function, that recieved the current project graph and returned a new one. As part of this, they could add new nodes and edges to the graph. Most nodes on the graph were projects, but adding a project via these APIs resulted in odd behavior as targets couldn't be ran because they weren't present when Nx initially loaded project configurations."}),`
